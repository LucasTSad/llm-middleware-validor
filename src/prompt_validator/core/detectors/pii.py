...
from prompt_validator.core.contracts import Finding, GuardConfig, NormalizedText, Category, Severity
...
class PiiDetector:
    ...
    def inspect(self, normalized_text: NormalizedText, config: GuardConfig) -> list[Finding]:
        ...
        for match in _CPF_PATTERN.finditer(normalized_text.normalized):
            ...
            if is_cpf(cpf):
                start, end = normalized_text.to_original_span(match.start(),
                                                                match.end())
